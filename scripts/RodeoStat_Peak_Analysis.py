...
#smooth curve
	yhat = signal.savgol_filter(current, 99, 3) # window size 51, polynomial order 3
...
#calculate first derivative
	...
	diff_smoothed = signal.savgol_filter(diff, 99, 3) # window size 51, polynomial order 3
...
#find peak maxima
	...
	max_value = np.argmax(np.array(current[:1300]))
	left_bound = max_value - 100
	right_bound = max_value + 100
	print(max_value)
	#use data points 50-500: eliminates noise at edges
	maxima = find_nearest(diff_smoothed[left_bound:right_bound], 0) 
...
#calculate baseline
	...
	x_linear = ewe_v_array[450:650] #ewe_v[625:650]
	y_linear = current_array[450:650] #current[625:650]
...
def runRodeoStatAnalysis(path, output_file):
	output = []
	output_df = pd.DataFrame(columns = ["Sample Name", "Peak Height"])
	for filename in path:
		if filename != output_file[0]:
			#df = pd.DataFrame(columns = ['time', 'Ewe/V', '<I>/mA'])
			df = skip_to(filename, "0")#, sep="\t")
			df.columns = ['time', 'Ewe/V', '<I>/mA']
			ewe_v = df['Ewe/V']
			current = df['<I>/mA']
			...
			output_df.loc[len(output_df)] = [ntpath.basename(filename), str(peak_height)]
	output_df.to_csv(output_file, index=False)
	output_df = output_df.set_index("Sample Name").T
	output_df.to_csv(output_file + "_transposed", index=False)
	return path
